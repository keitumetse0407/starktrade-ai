...
export default function AgentLiveView({ 
    ...
    switch (data.type) {
      case 'agent_status':
        setAgents(prev => prev.map(a => 
          a.id === data.agent_id ? { ...a, status: data.status, vote: data.vote, confidence: data.confidence, reasoning: data.reasoning } : a
        ));
        break;
      case 'system2_regime':
  ...
  // Demo mode: cycle through agent statuses for visual impact
  useEffect(() => {
    if (!demoMode) return;
    ...
    const demoInterval = setInterval(() => {
      setAgents(prev => {
        ...
        if (activeIdx === -1) {
          const newAgents = prev.map((a, i) => ({
            ...a,
            status: i === 0 ? 'analyzing' : 'idle',
            vote: undefined,
        ...
        const newStatus: Agent['status'] = 'voting';
        ...
        if (activeIdx < 6) {
          updated[activeIdx + 1] = { ...updated[activeIdx + 1], status: 'analyzing' };
        } else if (activeIdx === 6) {
          setTimeout(() => {
  ...
  return (
    <div className={`w-full ${className}`}>
          ...
          {agents.map((agent, idx) => (
            ...
            >
              <motion.div
                ...
                animate={agent.status === 'analyzing' ? { width: ['0%', '100%'] } : undefined}
                transition={agent.status === 'analyzing' ? { duration: 0.8, repeat: Infinity } : undefined}
              />
              ...
              <div className="p-3">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {agent.status === 'analyzing' && (
                    <motion.span className="text-[10px] text-[#00D4FF]" animate={{ opacity: [1, 0.5, 1] }} transition={{ duration: 0.5, repeat: Infinity }}>
                      ANALYZING
                    </motion.span>
                  )}
                ...
                {agent.vote && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
                    <span className="text-xs font-mono" style={{ color: agent.vote === 'BUY' ? '#00FF88' : agent.vote === 'SELL' ? '#FF4444' : '#FFD700' }}>
                      {agent.vote}
                    </span>
      ...
      <motion.div className="rounded-lg border mb-4 p-4" style={{ background: 'rgba(255,255,255,0.03)', borderColor: riskDecision === 'PENDING' ? 'rgba(0,212,255,0.15)' : riskDecision === 'APPROVED' ? '#00FF88' : '#FF4444' }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <motion.div className="text-xs text-white/40" animate={riskDecision === 'PENDING' ? { opacity: [1, 0.5, 1] } : undefined} transition={{ duration: 1, repeat: Infinity }}>
              {riskDecision === 'PENDING' ? 'EVALUATING...' : ''}
            </motion.div>
          </div>
          <div className="flex items-center gap-2">
      ...
      <AnimatePresence>
        {consensus && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg border p-4" style={{ background: 'rgba(0,212,cyan,0.05)', borderColor: '#00D4FF' }}>
            <div className="text-xs text-white/40 mb-2 font-mono">FINAL CONSENSUS</div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <span className="text-2xl font-mono font-bold" style={{ color: consensus.action === 'BUY' ? '#00FF88' : consensus.action === 'SELL' ? '#FF4444' : '#FFD700' }}>
                  {consensus.action}
                </span>
                <div>
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="px-4 py-2 rounded bg-[#00D4FF]/20 border text-sm font-mono" style={{ borderColor: '#00D4FF', color: '#00D4FF' }}>
                EXECUTE →
              </motion.button>
            </div>
          </motion.div>
